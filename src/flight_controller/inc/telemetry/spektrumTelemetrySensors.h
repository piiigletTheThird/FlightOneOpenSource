...
//////////////////////////////////////////////////////////////////
//
//	Copyright 2016 by Horizon Hobby, Inc.  
...
//////////////////////////////////////////////////////////////////////////////
//
//						TELEMETRY SENSOR I2C ADDRESSES
//
//////////////////////////////////////////////////////////////////////////////
//

#define	TELE_DEVICE_NODATA			(0x00)										// No data in packet, but telemetry is alive
#define	TELE_DEVICE_VOLTAGE			(0x01)										// High-Voltage sensor
#define	TELE_DEVICE_TEMPERATURE		(0x02)										// Temperature Sensor
...
#define	TELE_DEVICE_GMETER			(0x14)										// G-Meter (Eagle Tree Sensor)
#define	TELE_DEVICE_JETCAT			(0x15)										// JetCat interface (Eagle Tree)
#define	TELE_DEVICE_GPS_LOC			(0x16)										// GPS Location Data (Eagle Tree)
#define	TELE_DEVICE_GPS_STATS		(0x17)										// GPS Status (Eagle Tree)
#define	TELE_DEVICE_RX_MAH			(0x18)										// Receiver Pack Capacity
#define	TELE_DEVICE_JETCAT_2		(0x19)										// JetCat interface, message 2 format (Eagle Tree)
...
#define	TELE_DEVICE_ESC				(0x20)										// Electronic Speed Control (Kontronic Kosmic)
#define	TELE_DEVICE_FUEL			(0x22)										// Fuel Flow Meter
#define	TELE_DEVICE_ALPHA6			(0x24)										// Alpha6 Stabilizer
//#define	DO_NOT_USE				(0x30)										// Reserved for internal use
//#define	DO_NOT_USE				(0x32)										// Reserved for internal use
#define	TELE_DEVICE_FP_MAH			(0x34)										// Flight Battery Capacity
#define	TELE_DEVICE_DIGITAL_AIR		(0x36)										// Digital Inputs & Tank Pressure
#define	TELE_DEVICE_STRAIN			(0x38)										// Thrust/Strain Gauge
#define	TELE_DEVICE_LIPOMON			(0x3A)										// Cell Monitor (6 LiPo taps)
#define	TELE_DEVICE_LIPOMON_14		(0x3F)										// Cell Monitor (14 LiPo taps)
#define	TELE_DEVICE_VARIO_S			(0x40)										// Vario (simple)
#define	TELE_DEVICE_VARIO_TE		(0x43)										// Vario (Total Energy)
...
typedef struct
{
	...
	UINT32	lastLapTime;	// Time of lap in 1ms increments
	UINT32	gateTime;       //Time between individual gates
	UINT8   unused[4];
...
typedef struct
{
	...
	INT16		chargeUsed_B;													// Integrated mAh used, 0.1mAh (0-3276.6mAh)
...
typedef struct
{
	UINT8		identifier;														// Source device = 0x43
	UINT8		sID;															// Secondary ID
	UINT16		iStaticPressure,												// Range: 300 - 1100 mbar  Resolution: 16 Lsb/mbar
				iPitotPressure;
	UINT16		iStaticTemp,													// 64 LSb/degrees C
				iPitotTemp;
	INT8		staticRate1s,													// 1 sec rate		
				pitotRate1s,
				staticRate3s,													// 3 sec rate	
				pitotRate3s;
	INT16		airSpeed;														// difference pitot - static pressure
} STRU_TELE_VARIO_TE;
...
enum JETCAT_ECU_TURBINE_STATE {							// ECU Status definitions
	...
	JETCAT_ECU_STATE_WAIT_for_RPM, // (Stby/Start)
	...
	JETCAT_ECU_STATE_Run, // (reg.)
	JETCAT_ECU_STATE_Accleleration_delay,
	JETCAT_ECU_STATE_SpeedReg, // (Speed Ctrl)
	JETCAT_ECU_STATE_Two_Shaft_Regulate, // (only for turbines with secondary shaft)
...
typedef struct
{
	...
	INT16		temperature;													// Added per V3 spec 2010-01-06
} STRU_TELE_RPM;
